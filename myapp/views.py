...
def office_criteria(request):
    if request.method == "POST":
        # Check if the request is from "After Joining Formality"
        if "after_joining_request" in request.POST:
            if request.user.is_authenticated:
                user_name = request.user.get_full_name() or request.user.username
            else:
                user_name = "Unknown User"

            subject = "📄 Appointment Letter Request"
            message = f"🔹 Employee Name: {user_name}\n📄 Requested an Appointment Letter."
            ...
            try:
                email.send()
                messages.success(request, "Your request for an appointment letter has been submitted successfully!")
        ...
        # ✅ Handling Relieving Formality Request
        if "relieving_request" in request.POST:
            if request.user.is_authenticated:
                user_name = request.user.get_full_name() or request.user.username
            else:
                user_name = "Unknown User"

            subject = "📂 Relieving Request"
            message = f"🔹 Employee Name: {user_name}\n📄 Requested for relieving process initiation."
            ...
            try:
                email.send()
                messages.success(request, "Your request for relieving has been submitted successfully!")
            except Exception as e:
                messages.error(request, f"Email failed to send: {str(e)}")

            return redirect("office_criteria")

        # Existing functionality for Joining Formality
        ...
        if agreement == "agree":
            ...
            subject = "🚫 Not Joining Notification"
            message = f"🔹 Employee Name: {user_name}\n📝 Message: {text_data}\n⚠️ Not joining notification.\n"
