...
from .models import RelievingRule
...
def office_criteria(request):
    if request.method == 'POST':
        if 'rule' in request.POST or 'file' in request.FILES:
            # Handle adding a new office rule (text or file)
            rule_text = request.POST.get('rule', '').strip()
            uploaded_file = request.FILES.get('file', None)

            if rule_text or uploaded_file:
                Rule.objects.create(rule=rule_text, file=uploaded_file)
                return redirect('office_criteria')  # Refresh page to display the new rule

        elif 'delete-rule-id' in request.POST:
            # Handle deletion of a rule
            delete_rule_id = request.POST.get('delete-rule-id')
            if delete_rule_id:
                rule = get_object_or_404(Rule, id=delete_rule_id)
                
                # Delete the file from storage if it exists
                if rule.file and default_storage.exists(rule.file.name):
                    default_storage.delete(rule.file.name)

                rule.delete()
                return redirect('office_criteria')  # Refresh page after deletion

    # Fetch all office rules
    rules = Rule.objects.all()
    return render(request, 'office_criteria.html', {'rules': rules, 'is_admin': request.user.is_superuser})
...
def relieving(request):
    if request.method == 'POST':
        if request.user.is_superuser:
            if 'file' in request.FILES:
                ...
                RelievingRule.objects.create(file=uploaded_file)
                return redirect('relieving')

            elif 'delete-rule-id' in request.POST:
                # Handle file deletion
                rule_id = request.POST['delete-rule-id']
                rule_to_delete = get_object_or_404(RelievingRule, id=rule_id)

                # Delete the file from storage if it exists
                if rule_to_delete.file and default_storage.exists(rule_to_delete.file.name):
                    default_storage.delete(rule_to_delete.file.name)
    ...
    # Fetch all rules
    relieving_rules = RelievingRule.objects.all()
    return render(request, 'relieving.html', {'relieving_rules': relieving_rules})
...
def home(request):
    festivals = Festival.objects.all()  # Fetch festival data from the database
    return render(request, 'home.html', {'festivals': festivals})

from django.shortcuts import render, redirect

# Temporary storage for submitted employee data (use database in production)
employee_data = []
...
def employee_details(request):
    if request.method == "POST":
        ...
        # Store the data in a list (use a database in a real app)
...
def details(request):
    return render(request, "details.html", {"employees": employee_data})
